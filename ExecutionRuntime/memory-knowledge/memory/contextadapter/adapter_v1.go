...
const (
	memoryEnvelopeKindV1 = runtimeports.NamespacedNameV2("memory/context-envelope")
)

type AdapterV1 struct {
	owner contextsource.MemoryContextSourceCurrentReaderV2
	clock func() time.Time
}

var _ applicationports.ContextOwnerSourceReaderV1 = (*AdapterV1)(nil)

func NewAdapterV1(owner contextsource.MemoryContextSourceCurrentReaderV2, clock func() time.Time) (*AdapterV1, error) {
	if owner == nil {
...
func (a *AdapterV1) inspect(ctx context.Context, request applicationcontract.ContextOwnerSourceRequestV1) (applicationcontract.ContextOwnerSourceEnvelopeV1, contextsource.CurrentRequestV2, contextsource.CurrentProjectionV2, error) {
	...
	now := a.clock()
	if now.IsZero() || request.Owner != applicationcontract.ContextOwnerMemoryV1 || request.ValidateCurrent(now) != nil {
		return applicationcontract.ContextOwnerSourceEnvelopeV1{}, contextsource.CurrentRequestV2{}, contextsource.CurrentProjectionV2{}, ownercontract.ErrInvalidArgument
	}
	...
	sealed, err := memoryRequestForPhase(currentRequest, request)
	if err != nil {
		return applicationcontract.ContextOwnerSourceEnvelopeV1{}, contextsource.CurrentRequestV2{}, contextsource.CurrentProjectionV2{}, fmt.Errorf("memory Context adapter owner request: %w", err)
	}
	if !memoryRequestMatchesApplication(sealed, request) {
		return applicationcontract.ContextOwnerSourceEnvelopeV1{}, contextsource.CurrentRequestV2{}, contextsource.CurrentProjectionV2{}, fmt.Errorf("%w: memory Context adapter Session/Turn mapping", ownercontract.ErrEvidenceConflict)
	}
	...
	if err != nil || projection.Validate() != nil || !sameMemoryCoordinate(projection.Coordinate, currentRequest.Coordinate) {
		if err != nil {
			return applicationcontract.ContextOwnerSourceEnvelopeV1{}, contextsource.CurrentRequestV2{}, contextsource.CurrentProjectionV2{}, err
		}
	...
	for index, item := range projection.Items {
		converted, convertErr := memoryItem(item, projection.SensitivityMax)
		if convertErr != nil {
	...
	envelope, err := applicationcontract.SealContextOwnerSourceEnvelopeV1(applicationcontract.ContextOwnerSourceEnvelopeV1{
		ID: "memory-envelope:" + projection.Ref.Digest, Owner: applicationcontract.ContextOwnerMemoryV1,
		...
		AttemptInspectionRef: ownerRef("memory/inspection", inspection.Ref), CurrentProjectionRef: ownerRef("memory/projection", projection.Ref),
...
func (a *AdapterV1) ReadContextOwnerContentExactV1(ctx context.Context, request applicationcontract.ContextOwnerContentRequestV1) (applicationcontract.ContextOwnerContentObservationV1, []byte, error) {
	...
	now := a.clock()
	if now.IsZero() || request.ValidateCurrent(now) != nil || request.SourceRequest.Owner != applicationcontract.ContextOwnerMemoryV1 {
		return applicationcontract.ContextOwnerContentObservationV1{}, nil, ownercontract.ErrInvalidArgument
	}
	...
	if ownerObservation.Validate() != nil || int64(len(body)) != ownerObservation.ObservedLength || core.DigestBytes(body) != core.Digest(ownerObservation.ObservedDigest) || !bytes.Equal([]byte(ownerObservation.ContentRef.Digest), []byte(projection.Items[request.Rank].ContentRef.Digest)) {
		return applicationcontract.ContextOwnerContentObservationV1{}, nil, ownercontract.ErrEvidenceConflict
	}
	completed := a.clock()
	...
	observation, err := applicationcontract.SealContextOwnerContentObservationV1(applicationcontract.ContextOwnerContentObservationV1{
		ID: "memory-content-observation:" + ownerObservation.Ref.Digest, Owner: applicationcontract.ContextOwnerMemoryV1,
		EnvelopeRef:          applicationcontract.ContextRefreshExactRefV1{Kind: memoryEnvelopeKindV1, ID: fresh.ID, Revision: fresh.Revision, Digest: fresh.Digest},
...
func memoryRequestMatchesApplication(owner contextsource.CurrentRequestV2, app applicationcontract.ContextOwnerSourceRequestV1) bool {
...
func memoryRequestForPhase(owner contextsource.CurrentRequestV2, app applicationcontract.ContextOwnerSourceRequestV1) (contextsource.CurrentRequestV2, error) {
...
func memoryItem(item contextsource.ProjectionItemV2, sensitivity string) (applicationcontract.ContextOwnerSourceItemV1, error) {
	chain := make([]applicationcontract.ContextRefreshExactRefV1, 0, len(item.SourceRefs)+len(item.EvidenceRefs)+len(item.ProjectionRefs)+2)
	for _, ref := range item.SourceRefs {
		chain = append(chain, ownerRef("memory/source", ref))
	}
	for _, ref := range item.EvidenceRefs {
		chain = append(chain, ownerRef("memory/evidence", ref))
	}
	for _, ref := range item.ProjectionRefs {
		chain = append(chain, ownerRef("memory/projection", ref))
	}
	chain = append(chain, ownerRef("memory/domain-result", item.DomainResultRef), ownerRef("memory/application", item.ApplicationRef))
	converted, err := applicationcontract.SealContextOwnerSourceItemV1(applicationcontract.ContextOwnerSourceItemV1{
		Rank: uint32(item.Rank), ItemDigest: core.Digest(item.Digest), RecordRef: ownerRef("memory/record", item.RecordRef), StableOwnerChain: chain,
		ContentRef: ownerContentRef(item.ContentRef), TokenEstimate: uint64(item.TokenEstimate), Sensitivity: sensitivity,
		CitationDigest: core.Digest(item.CitationDigest), ExpiresUnixNano: item.ExpiresAt.UnixNano(),
	})
...
func sameMemoryCoordinate(a, b contextsource.AttemptCoordinateV2) bool {
...
func invalid(format string, values ...any) error {
	return fmt.Errorf("%w: "+format, append([]any{ownercontract.ErrInvalidArgument}, values...)...)
}
