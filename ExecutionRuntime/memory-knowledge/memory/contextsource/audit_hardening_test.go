...
func TestMemoryCurrentReaderFreshOwnerClockAndTurnBinding(t *testing.T) {
	...
	t.Run("exact read uses fresh owner now", func(t *testing.T) {
		...
		req := exactRequest(f, projection, f.now.Add(time.Minute))
		if _, _, err := f.store.ReadContentExact(req); !errors.Is(err, contract.ErrNotCurrent) {
...
func TestMemoryCurrentReaderCanonicalDiscriminatorsAndSemanticKeys(t *testing.T) {
	t.Run("version and object kind", func(t *testing.T) {
		...
		badState.ObjectKind = "knowledge_current_state"
		...
		wrongProjection.ObjectKind = "knowledge_contribution_current_projection"
	...
	t.Run("nested refs and duplicate semantic record", func(t *testing.T) {
		...
		bad.Items[0].SourceRefs[0] = contract.Ref{}
		if _, err := f.store.PutAttempt(bad, contract.ExpectRevision(1)); err == nil {
			t.Fatal("invalid nested Source ref accepted")
		}
		...
		badCurrent.Items[0].ProjectionRefs[0] = contract.Ref{}
		if _, err := f.store.PublishCurrent(badCurrent, contract.ExpectRevision(1)); err == nil {
			t.Fatal("invalid nested current Projection ref accepted")
		}
	})
	...
	t.Run("rank derived from semantic order", func(t *testing.T) {
		...
		low.Rank = 99
		low.Score = 1
		low.RecordRef = testRef("memory/record-low")
		high := candidate.Items[0]
		high.Rank = 77
		high.Score = 100
		high.RecordRef = testRef("memory/record-high")
		candidate.Items = []StoredItem{low, high}
		sealed, err := f.store.PutAttempt(candidate, contract.ExpectRevision(1))
		if err != nil {
			t.Fatal(err)
		}
		if sealed.Items[0].RecordRef.ID != "memory/record-high" || sealed.Items[0].Rank != 0 || sealed.Items[1].Rank != 1 {
			t.Fatalf("caller rank influenced canonical order: %+v", sealed.Items)
		}
	})
}

func TestMemoryCurrentReaderStatePlaneBindingAndInspectClassification(t *testing.T) {
	t.Run("remote or tampered binding rejected", func(t *testing.T) {
		now := time.Date(2026, 7, 16, 2, 0, 0, 0, time.UTC)
		binding, err := NewStatePlaneBinding("memory/binding", 1, "memory-owner-state-plane", now.Add(time.Hour))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("missing id differs from exact ref drift", func(t *testing.T) {
		...
		missing.AttemptRef = testRef("memory/missing-attempt")
...
func TestMemoryCurrentReaderBarrierTTLAndBinding(t *testing.T) {
	...
	t.Run("Get binding drift returns no body", func(t *testing.T) {
		...
		<-started
		drift, err := NewStatePlaneBinding("memory/state-plane-binding", 2, "memory-owner-state-plane", f.now.Add(time.Hour))
		if err != nil {
			t.Fatal(err)
		}
...
func TestMemoryStatePlaneContentStoreCopyAndEvict(t *testing.T) {
	now := time.Date(2026, 7, 16, 2, 0, 0, 0, time.UTC)
	binding, err := NewStatePlaneBinding("memory/reference-binding", 1, "memory-owner-state-plane", now.Add(time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	...
	body := []byte("immutable memory bytes")
	...
	if err != nil || string(first) != "immutable memory bytes" {
		t.Fatalf("first=%q err=%v", first, err)
	}
	first[0] = 'Y'
	second, err := store.Get(ref)
	if err != nil || string(second) != "immutable memory bytes" {
