...
func newReaderFixtureV2(t *testing.T) *readerFixtureV2 {
	...
	coordinate := AttemptCoordinateV2{
		...
		TenantID: attempt.TenantID, IdentityRef: testRef(attempt.IdentityID), IdentityEpoch: 7,
	...
	req := CurrentRequestV2{
		...
		CurrentStateRef: base.state.Ref, ExpectedQueryRef: attempt.QueryRef, ExpectedViewRef: attempt.ViewRef, ExpectedWatermarkRef: attempt.WatermarkRef,
		AuthorityRef: attempt.AuthorityRef, AuthorityEpoch: attempt.AuthorityEpoch, PolicyRef: attempt.PolicyRef, Purpose: attempt.Purpose, Scopes: append([]string{}, attempt.Scopes...), SensitivityMax: attempt.SensitivityMax,
		CheckPhase: CheckPhaseS1V2, MaxItems: 8, MaxBytes: 4096, MaxTokens: 1024, PerItemMaxBytes: 1024, EstimatorRef: testRef("estimator"),
		CheckedUpperBound: base.now, NotAfter: base.now.Add(30 * time.Minute), ProjectionID: "memory/contribution/v2/s1", ProjectionRevision: 1,
	}
...
func TestMemoryCurrentReaderV2StableFreshBoundedAndCopyIsolation(t *testing.T) {
	...
	if err != nil || !s1.Current || len(s1.Items) != 1 {
	...
	s2req.ProjectionID = "memory/contribution/v2/s2"
	...
	if err != nil || string(body) != "owner local memory content" || observation.ObservedDigest != s2.Items[0].ContentRef.Digest {
...
func TestMemoryCurrentReaderV2CancellationTTLCurrentnessAndPoison(t *testing.T) {
	t.Run("lost reply inspect and exact drift classification", func(t *testing.T) {
		...
		missing.AttemptRef = testRef("memory/missing-v2-attempt")
		missing.Digest = ""
	...
	t.Run("owner ttl beats caller upper bound", func(t *testing.T) {
		...
		if _, err := f.reader.InspectForTurn(context.Background(), f.request); !errors.Is(err, contract.ErrNotCurrent) {
			t.Fatalf("expired turn accepted: %v", err)
		}
	})
...
func TestMemoryCurrentReaderV2GetCrossesTTLAnd64Concurrent(t *testing.T) {
	f := newReaderFixtureV2(t)
	p, err := f.reader.InspectForTurn(context.Background(), f.request)
	if err != nil {
		t.Fatal(err)
