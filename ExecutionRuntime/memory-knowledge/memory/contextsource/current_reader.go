// Package contextsource implements the Memory Owner-local current reader.
...
const (
	ContractVersion             = "praxis.memory/context-source-current-reader/v1"
	localAttemptKind            = "memory_local_retrieval_attempt"
	currentStateKind            = "memory_current_state"
	attemptInspectionKind       = "memory_local_attempt_inspection"
	currentProjectionKind       = "memory_contribution_current_projection"
	exactContentObservationKind = "memory_local_exact_content_observation"
	statePlaneBindingKind       = "memory_owner_local_state_plane_binding"
...
type MemoryContextSourceCurrentReaderV1 interface {
...
var _ MemoryContextSourceCurrentReaderV1 = (*Store)(nil)
...
func (s *Store) PutAttempt(in LocalAttempt, expected contract.ExpectedRevision) (LocalAttempt, error) {
	...
	if !expected.Matches(len(versions) != 0, lastAttemptRevision(versions)) || sealed.Ref.Revision != lastAttemptRevision(versions)+1 {
...
func (s *Store) PublishCurrent(in CurrentState, expected contract.ExpectedRevision) (CurrentState, error) {
	...
	if !expected.Matches(len(versions) != 0, lastStateRevision(versions)) || sealed.Ref.Revision != lastStateRevision(versions)+1 {
...
func (s *Store) InspectAttempt(c AttemptCoordinate) (AttemptInspection, error) {
	...
	if len(versions) == 0 {
		return sealInspection(AttemptInspection{ContractVersion: ContractVersion, ObjectKind: attemptInspectionKind, Ref: contract.Ref{ID: "memory/context-inspection/" + c.AttemptRef.ID, Revision: c.AttemptRef.Revision}, Owner: contract.OwnerMemory, AttemptRef: c.AttemptRef, RunID: c.RunID, TurnID: c.TurnID, Status: AttemptNotPersisted, ObservedAt: checked, ExpiresAt: checked.Add(time.Minute)})
	}
	if !found {
	...
	if attempt.TenantID != c.TenantID || attempt.IdentityID != c.IdentityID || attempt.ExecutionScopeDigest != c.ExecutionScopeDigest || attempt.RunID != c.RunID || attempt.TurnID != c.TurnID || attempt.RequestDigest != c.RequestDigest || attempt.IdempotencyKey != c.IdempotencyKey || !contract.SameRef(attempt.ObservationRef, c.ObservationRef) || !contract.SameRef(attempt.ResultRef, c.ResultRef) {
		return AttemptInspection{}, contract.ErrEvidenceConflict
	}
	...
	expires := minTime(attempt.ExpiresAt, checked.Add(time.Minute))
	return sealInspection(AttemptInspection{ContractVersion: ContractVersion, ObjectKind: attemptInspectionKind, Ref: contract.Ref{ID: "memory/context-inspection/" + attempt.Ref.ID, Revision: attempt.Ref.Revision}, Owner: contract.OwnerMemory, AttemptRef: attempt.Ref, RunID: attempt.RunID, TurnID: attempt.TurnID, ObservationRef: attempt.ObservationRef, ResultRef: attempt.ResultRef, Status: status, ObservedAt: checked, ExpiresAt: expires})
}
...
func (s *Store) ReadContentExact(req ExactContentRequest) (ExactContentObservation, []byte, error) {
	...
	body = bytes.Clone(body)
	expires := minTime(req.Projection.ExpiresAt, req.NotAfter)
	observation := ExactContentObservation{ContractVersion: ContractVersion, ObjectKind: exactContentObservationKind, Ref: contract.Ref{ID: "memory/content-observation/" + req.Projection.Ref.ID + fmt.Sprintf("/%d", req.Rank), Revision: req.Projection.Ref.Revision}, Owner: contract.OwnerMemory, ProjectionRef: req.Projection.Ref, StatePlaneBindingRef: s.binding.Ref, ClosureDigest: req.Projection.ClosureDigest, RunID: req.RunID, TurnID: req.TurnID, Rank: req.Rank, RecordRef: item.RecordRef, ContentRef: item.ContentRef, ObservedLength: int64(len(body)), ObservedMedia: item.ContentRef.MediaType, ObservedDigest: digestBytes(body), ObservedAt: s2Now, ExpiresAt: expires}
...
func (s *Store) recheckExactLocked(req ExactContentRequest, ownerNow time.Time) error {
	...
	_, err := buildProjection(attempt, state, CurrentRequest{ContractVersion: ContractVersion, Coordinate: coordinateFrom(attempt), RunID: req.RunID, TurnID: req.TurnID, CurrentStateRef: state.Ref, AuthorityRef: req.Projection.AuthorityRef, AuthorityEpoch: req.Projection.AuthorityEpoch, PolicyRef: req.Projection.PolicyRef, Purpose: req.Projection.Purpose, Scopes: req.Projection.Scopes, SensitivityMax: req.Projection.SensitivityMax, CheckedAt: req.CheckedAt, NotAfter: req.Projection.ExpiresAt, ProjectionID: req.Projection.Ref.ID, ProjectionRevision: req.Projection.Ref.Revision, ExpectedS1ClosureDigest: req.Projection.ClosureDigest}, ownerNow, s.binding)
	return err
}
...
func buildProjection(attempt LocalAttempt, state CurrentState, req CurrentRequest, ownerNow time.Time, binding StatePlaneBinding) (CurrentProjection, error) {
	...
	for _, stored := range attempt.Items {
		...
		if !ok || !current.Active || !current.PoisoningCleared || !contract.SameRef(current.RecordRef, stored.RecordRef) || current.ContentRef != stored.ContentRef || !sameRefs(current.ProjectionRefs, stored.ProjectionRefs) {
			return CurrentProjection{}, contract.ErrNotCurrent
		}
		itemExpires := minTime(stored.RecordExpiresAt, stored.ProjectionExpires, current.RecordExpiresAt, current.ProjectionExpires, binding.ExpiresAt, req.NotAfter)
		if !itemExpires.After(ownerNow) {
			return CurrentProjection{}, contract.ErrNotCurrent
		}
		expires = minTime(expires, itemExpires)
		items = append(items, ProjectionItem{Rank: stored.Rank, Score: stored.Score, RecordRef: stored.RecordRef, ContentRef: stored.ContentRef, SourceRefs: slices.Clone(stored.SourceRefs), EvidenceRefs: slices.Clone(stored.EvidenceRefs), ProjectionRefs: slices.Clone(stored.ProjectionRefs), CitationDigest: stored.CitationDigest, DomainResultRef: attempt.DomainResult.Ref, ApplicationRef: attempt.Application.Ref, ExpiresAt: itemExpires})
	}
	projection := CurrentProjection{ContractVersion: ContractVersion, ObjectKind: currentProjectionKind, Ref: contract.Ref{ID: req.ProjectionID, Revision: req.ProjectionRevision}, Owner: contract.OwnerMemory, Current: true, AttemptRef: attempt.Ref, ObservationRef: attempt.ObservationRef, ResultRef: attempt.ResultRef, CurrentStateRef: state.Ref, StatePlaneBindingRef: binding.Ref, RunID: req.RunID, TurnID: req.TurnID, ViewRef: state.ViewRef, WatermarkRef: state.WatermarkRef, AuthorityRef: state.AuthorityRef, AuthorityEpoch: state.AuthorityEpoch, PolicyRef: state.PolicyRef, Purpose: state.Purpose, Scopes: slices.Clone(state.Scopes), SensitivityMax: state.SensitivityMax, Coverage: cloneCoverage(attempt.Coverage), Items: items, CheckedAt: ownerNow, ExpiresAt: expires}
...
func sealAttempt(in LocalAttempt) (LocalAttempt, error) {
	...
	in.Coverage = cloneCoverage(in.Coverage)
	in.Coverage.ProjectionRefs = contract.NormalizeRefs(in.Coverage.ProjectionRefs)
	in.Coverage.DroppedReasons = normalizeStrings(in.Coverage.DroppedReasons)
	...
	if err != nil {
		return LocalAttempt{}, fmt.Errorf("canonical memory local attempt: %w", err)
	}
...
func validateAttempt(in LocalAttempt) error {
	if in.ContractVersion != ContractVersion || in.ObjectKind != localAttemptKind || in.Owner != contract.OwnerMemory || strings.TrimSpace(in.Ref.ID) == "" || in.Ref.Revision == 0 || strings.TrimSpace(in.Ref.Digest) == "" || strings.TrimSpace(in.TenantID) == "" || strings.TrimSpace(in.IdentityID) == "" || strings.TrimSpace(in.ExecutionScopeDigest) == "" || strings.TrimSpace(in.RunID) == "" || strings.TrimSpace(in.TurnID) == "" || strings.TrimSpace(in.RequestDigest) == "" || strings.TrimSpace(in.IdempotencyKey) == "" || strings.TrimSpace(in.Purpose) == "" || len(in.Scopes) == 0 || strings.TrimSpace(in.SensitivityMax) == "" || in.AuthorityEpoch == 0 || in.ObservedAt.IsZero() || in.ExpiresAt.IsZero() || len(in.Items) > 64 || hasDuplicateStoredRecord(in.Items) {
		return contract.ErrInvalidArgument
	}
	for _, ref := range []contract.Ref{in.ObservationRef, in.ResultRef, in.QueryRef, in.ViewRef, in.WatermarkRef, in.AuthorityRef, in.PolicyRef} {
	...
	for i, item := range in.Items {
		if item.Rank != i || (i > 0 && compareStoredItems(in.Items[i-1], item) >= 0) || item.RecordRef.Validate() != nil || item.ContentRef.Validate() != nil || validateRefs(item.SourceRefs, true) != nil || validateRefs(item.EvidenceRefs, true) != nil || validateRefs(item.ProjectionRefs, true) != nil || strings.TrimSpace(item.CitationDigest) == "" || !item.RecordExpiresAt.After(in.ObservedAt) || !item.ProjectionExpires.After(in.ObservedAt) {
			return contract.ErrInvalidArgument
		}
	}
	return nil
...
func validateSettlement(in LocalAttempt) error {
	if in.DomainResult.Owner != contract.OwnerMemory || !contract.SameRef(in.DomainResult.SubjectRef, in.ResultRef) {
	...
	if in.Application.Owner != contract.OwnerMemory || !contract.SameRef(in.Application.DomainResultRef, in.DomainResult.Ref) {
		return contract.ErrSettlementMismatch
	}
	return nil
}
...
func sealCurrentState(in CurrentState) (CurrentState, error) {
	...
	digest, err := contract.Digest(in)
	if err != nil {
		return CurrentState{}, fmt.Errorf("canonical memory current state: %w", err)
	}
	in.Ref.Digest = digest
...
func validateCurrentState(in CurrentState) error {
	if in.ContractVersion != ContractVersion || in.ObjectKind != currentStateKind || in.Owner != contract.OwnerMemory || strings.TrimSpace(in.Ref.ID) == "" || in.Ref.Revision == 0 || strings.TrimSpace(in.Ref.Digest) == "" || strings.TrimSpace(in.TenantID) == "" || strings.TrimSpace(in.IdentityID) == "" || in.AuthorityEpoch == 0 || strings.TrimSpace(in.Purpose) == "" || len(in.Scopes) == 0 || strings.TrimSpace(in.SensitivityMax) == "" || in.ExpiresAt.IsZero() || hasDuplicateCurrentRecord(in.Items) {
		return contract.ErrInvalidArgument
	}
	for _, ref := range []contract.Ref{in.AuthorityRef, in.PolicyRef, in.ViewRef, in.WatermarkRef} {
		if err := ref.Validate(); err != nil {
			return err
		}
	}
	...
	for i, item := range in.Items {
		if (i > 0 && compareCurrentItems(in.Items[i-1], item) >= 0) || item.RecordRef.Validate() != nil || item.ContentRef.Validate() != nil || validateRefs(item.ProjectionRefs, true) != nil || !item.RecordExpiresAt.After(time.Time{}) || !item.ProjectionExpires.After(time.Time{}) {
			return contract.ErrInvalidArgument
		}
	}
	return nil
...
func validateCoordinate(c AttemptCoordinate) error {
	if c.ContractVersion != ContractVersion || strings.TrimSpace(c.TenantID) == "" || strings.TrimSpace(c.IdentityID) == "" || strings.TrimSpace(c.ExecutionScopeDigest) == "" || strings.TrimSpace(c.RunID) == "" || strings.TrimSpace(c.TurnID) == "" || strings.TrimSpace(c.RequestDigest) == "" || strings.TrimSpace(c.IdempotencyKey) == "" {
		return contract.ErrInvalidArgument
	}
...
func validateCurrentRequest(req CurrentRequest) error {
	...
	if req.ContractVersion != ContractVersion || strings.TrimSpace(req.RunID) == "" || strings.TrimSpace(req.TurnID) == "" || req.RunID != req.Coordinate.RunID || req.TurnID != req.Coordinate.TurnID || req.CurrentStateRef.Validate() != nil || req.AuthorityRef.Validate() != nil || req.PolicyRef.Validate() != nil || req.AuthorityEpoch == 0 || strings.TrimSpace(req.Purpose) == "" || len(req.Scopes) == 0 || strings.TrimSpace(req.SensitivityMax) == "" || req.CheckedAt.IsZero() || !req.NotAfter.After(req.CheckedAt) || strings.TrimSpace(req.ProjectionID) == "" || req.ProjectionRevision == 0 {
		return contract.ErrInvalidArgument
	}
	return nil
}
...
func validateProjection(in CurrentProjection) error {
	if in.ContractVersion != ContractVersion || in.ObjectKind != currentProjectionKind || in.Owner != contract.OwnerMemory || !in.Current || in.Ref.Validate() != nil || in.AttemptRef.Validate() != nil || in.ObservationRef.Validate() != nil || in.ResultRef.Validate() != nil || in.CurrentStateRef.Validate() != nil || in.StatePlaneBindingRef.Validate() != nil || strings.TrimSpace(in.RunID) == "" || strings.TrimSpace(in.TurnID) == "" || in.ViewRef.Validate() != nil || in.WatermarkRef.Validate() != nil || in.AuthorityRef.Validate() != nil || in.PolicyRef.Validate() != nil || in.AuthorityEpoch == 0 || strings.TrimSpace(in.Purpose) == "" || len(in.Scopes) == 0 || strings.TrimSpace(in.SensitivityMax) == "" || strings.TrimSpace(in.ClosureDigest) == "" || in.CheckedAt.IsZero() || !in.ExpiresAt.After(in.CheckedAt) || hasDuplicateProjectionRecord(in.Items) {
		return contract.ErrInvalidArgument
	}
	copy := in
	copy.Ref.Digest = ""
	...
	for i, item := range in.Items {
		if item.Rank != i || (i > 0 && compareProjectionItems(in.Items[i-1], item) >= 0) || item.RecordRef.Validate() != nil || item.ContentRef.Validate() != nil || item.DomainResultRef.Validate() != nil || item.ApplicationRef.Validate() != nil || validateRefs(item.SourceRefs, true) != nil || validateRefs(item.EvidenceRefs, true) != nil || validateRefs(item.ProjectionRefs, true) != nil {
			return contract.ErrInvalidArgument
		}
	}
	return nil
...
func closureDigest(in CurrentProjection) (string, error) {
	...
	return contract.Digest(closure{
		ContractVersion: in.ContractVersion, ObjectKind: in.ObjectKind,
		Owner: in.Owner, AttemptRef: in.AttemptRef, ObservationRef: in.ObservationRef,
		ResultRef: in.ResultRef, CurrentStateRef: in.CurrentStateRef, StatePlaneBindingRef: in.StatePlaneBindingRef, ViewRef: in.ViewRef,
		RunID: in.RunID, TurnID: in.TurnID,
		WatermarkRef: in.WatermarkRef, AuthorityRef: in.AuthorityRef, AuthorityEpoch: in.AuthorityEpoch,
		PolicyRef: in.PolicyRef, Purpose: in.Purpose, Scopes: normalizeStrings(in.Scopes),
		SensitivityMax: in.SensitivityMax, Coverage: cloneCoverage(in.Coverage),
		Items: cloneProjectionItems(in.Items), ExpiresAt: in.ExpiresAt,
	})
}
...
func sealInspection(in AttemptInspection) (AttemptInspection, error) {
	...
	in.Ref.Digest = digest
	if in.ContractVersion != ContractVersion || in.ObjectKind != attemptInspectionKind || in.Owner != contract.OwnerMemory || in.Ref.Validate() != nil || in.AttemptRef.Validate() != nil || strings.TrimSpace(in.RunID) == "" || strings.TrimSpace(in.TurnID) == "" || in.ObservedAt.IsZero() || !in.ExpiresAt.After(in.ObservedAt) {
...
func sealContentObservation(in ExactContentObservation) (ExactContentObservation, error) {
	...
	in.Ref.Digest = digest
	if in.ContractVersion != ContractVersion || in.ObjectKind != exactContentObservationKind || in.Owner != contract.OwnerMemory || in.Ref.Validate() != nil || in.ProjectionRef.Validate() != nil || in.StatePlaneBindingRef.Validate() != nil || strings.TrimSpace(in.ClosureDigest) == "" || strings.TrimSpace(in.RunID) == "" || strings.TrimSpace(in.TurnID) == "" || in.RecordRef.Validate() != nil || in.ContentRef.Validate() != nil || in.ObservedAt.IsZero() || !in.ExpiresAt.After(in.ObservedAt) || in.ObservedLength != in.ContentRef.Length || in.ObservedDigest != in.ContentRef.Digest || in.ObservedMedia != in.ContentRef.MediaType {
...
func coordinateFrom(in LocalAttempt) AttemptCoordinate {
	return AttemptCoordinate{ContractVersion: ContractVersion, TenantID: in.TenantID, IdentityID: in.IdentityID, ExecutionScopeDigest: in.ExecutionScopeDigest, RunID: in.RunID, TurnID: in.TurnID, AttemptRef: in.Ref, RequestDigest: in.RequestDigest, IdempotencyKey: in.IdempotencyKey, ObservationRef: in.ObservationRef, ResultRef: in.ResultRef}
}

func NewStatePlaneBinding(id string, revision uint64, storeDomain string, expiresAt time.Time) (StatePlaneBinding, error) {
	in := StatePlaneBinding{ContractVersion: ContractVersion, ObjectKind: statePlaneBindingKind, Ref: contract.Ref{ID: id, Revision: revision}, Owner: contract.OwnerMemory, StoreDomain: storeDomain, AccessKind: localExactAccessKind, ExpiresAt: expiresAt.UTC()}
	in.Ref.Digest = ""
	digest, err := contract.Digest(in)
	if err != nil {
		return StatePlaneBinding{}, err
...
func validateStatePlaneBinding(in StatePlaneBinding, ownerNow time.Time) error {
	if in.ContractVersion != ContractVersion || in.ObjectKind != statePlaneBindingKind || in.Owner != contract.OwnerMemory || in.Ref.Validate() != nil || strings.TrimSpace(in.StoreDomain) == "" || in.AccessKind != localExactAccessKind || strings.TrimSpace(in.RemoteLocator) != "" || in.ExpiresAt.IsZero() {
		return contract.ErrInvalidArgument
	}
	copy := in
	copy.Ref.Digest = ""
