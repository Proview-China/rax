...
const (
	ContractVersionV2             = "praxis.memory/context-source-current-reader/v2"
	AttemptCoordinateKindV2       = "memory_context_source_attempt_coordinate"
	AttemptInspectionKindV2       = "memory_local_attempt_inspection"
	CurrentRequestKindV2          = "memory_context_source_current_request"
	ProjectionItemKindV2          = "memory_context_source_projection_item"
	CurrentProjectionKindV2       = "memory_contribution_current_projection"
	ExactContentRequestKindV2     = "memory_local_exact_content_request"
	ExactContentObservationKindV2 = "memory_local_exact_content_observation"
...
type MemoryContextSourceCurrentReaderV2 interface {
...
var _ MemoryContextSourceCurrentReaderV2 = (*CurrentReaderV2)(nil)
...
func (r *CurrentReaderV2) InspectAttempt(ctx context.Context, coordinate AttemptCoordinateV2) (AttemptInspectionV2, error) {
	...
	if len(coordinates) == 0 {
		return sealAttemptInspectionV2(AttemptInspectionV2{ContractVersion: ContractVersionV2, ObjectKind: AttemptInspectionKindV2, Ref: contract.Ref{ID: "memory/context-inspection/v2/" + coordinate.AttemptRef.ID, Revision: coordinate.AttemptRef.Revision}, Owner: contract.OwnerMemory, Coordinate: coordinate, Status: AttemptNotPersisted, OwnerCheckedAt: now, ExpiresAt: minTime(now.Add(time.Minute), minTime(coordinate.SessionExpiresAt, coordinate.TurnExpiresAt))})
	}
	...
	return sealAttemptInspectionV2(AttemptInspectionV2{ContractVersion: ContractVersionV2, ObjectKind: AttemptInspectionKindV2, Ref: contract.Ref{ID: "memory/context-inspection/v2/" + attempt.Ref.ID, Revision: attempt.Ref.Revision}, Owner: contract.OwnerMemory, Coordinate: stored, ObservationRef: &observation, ResultRef: &result, Status: status, OwnerCheckedAt: now, ExpiresAt: minTime(attempt.ExpiresAt, minTime(stored.SessionExpiresAt, minTime(stored.TurnExpiresAt, now.Add(time.Minute))))})
}
...
func (r *CurrentReaderV2) ReadContentExact(ctx context.Context, req ExactContentRequestV2) (ExactContentObservationV2, []byte, error) {
	...
	observation := ExactContentObservationV2{ContractVersion: ContractVersionV2, ObjectKind: ExactContentObservationKindV2, Ref: contract.Ref{ID: "memory/content-observation/v2/" + req.Projection.Ref.ID + fmt.Sprintf("/%d/%s", req.Rank, req.CheckPhase), Revision: req.Projection.Ref.Revision}, Owner: contract.OwnerMemory, ProjectionRef: req.Projection.Ref, ProjectionItemDigest: item.Digest, StatePlaneBindingRef: after.StatePlaneBindingRef, StableClosureDigest: after.StableClosureDigest, Coordinate: after.Coordinate, Rank: item.Rank, RecordRef: item.RecordRef, ContentRef: item.ContentRef, DomainResultRef: item.DomainResultRef, AssociationDigest: item.AssociationDigest, ApplicationRef: item.ApplicationRef, ObservedLength: int64(len(body)), ObservedMediaType: item.ContentRef.MediaType, ObservedDigest: digestBytes(body), CheckPhase: req.CheckPhase, OwnerObservedAt: s2Now, ExpiresAt: minTime(after.ExpiresAt, req.NotAfter)}
...
func (r *CurrentReaderV2) buildProjectionLocked(ctx context.Context, req CurrentRequestV2, now time.Time) (CurrentProjectionV2, error) {
	...
	v1req := CurrentRequest{ContractVersion: ContractVersion, Coordinate: coordinateFrom(attempt), RunID: attempt.RunID, TurnID: attempt.TurnID, CurrentStateRef: state.Ref, AuthorityRef: req.AuthorityRef, AuthorityEpoch: req.AuthorityEpoch, PolicyRef: req.PolicyRef, Purpose: req.Purpose, Scopes: req.Scopes, SensitivityMax: req.SensitivityMax, CheckedAt: req.CheckedUpperBound, NotAfter: req.NotAfter, ProjectionID: req.ProjectionID + "/v1-validation", ProjectionRevision: req.ProjectionRevision}
	v1, err := buildProjection(attempt, state, v1req, now, binding)
	if err != nil {
		return CurrentProjectionV2{}, err
	}
	if !contract.SameRef(req.ExpectedQueryRef, attempt.QueryRef) || !contract.SameRef(req.ExpectedViewRef, attempt.ViewRef) || !contract.SameRef(req.ExpectedWatermarkRef, attempt.WatermarkRef) {
		return CurrentProjectionV2{}, contract.ErrEvidenceConflict
	}
	...
	for _, item := range v1.Items {
		...
		v2item := ProjectionItemV2{ContractVersion: ContractVersionV2, ObjectKind: ProjectionItemKindV2, Rank: len(items), Score: item.Score, RecordRef: item.RecordRef, ContentRef: item.ContentRef, SourceRefs: contract.NormalizeRefs(item.SourceRefs), EvidenceRefs: contract.NormalizeRefs(item.EvidenceRefs), ProjectionRefs: contract.NormalizeRefs(item.ProjectionRefs), CitationDigest: item.CitationDigest, DomainResultRef: item.DomainResultRef, AssociationDigest: associationDigest, ApplicationRef: item.ApplicationRef, TokenEstimate: tokens, EstimatorRef: req.EstimatorRef, ExpiresAt: item.ExpiresAt}
...
func (r *CurrentReaderV2) rebuildProjectionLocked(ctx context.Context, projection CurrentProjectionV2, now time.Time) (CurrentProjectionV2, error) {
	req := CurrentRequestV2{ContractVersion: ContractVersionV2, ObjectKind: CurrentRequestKindV2, Coordinate: projection.Coordinate, CurrentStateRef: projection.CurrentStateRef, ExpectedQueryRef: projection.QueryRef, ExpectedViewRef: projection.ViewRef, ExpectedWatermarkRef: projection.WatermarkRef, AuthorityRef: projection.AuthorityRef, AuthorityEpoch: projection.AuthorityEpoch, PolicyRef: projection.PolicyRef, Purpose: projection.Purpose, Scopes: projection.Scopes, SensitivityMax: projection.SensitivityMax, CheckPhase: projection.CheckPhase, ExpectedS1ClosureDigest: projection.StableClosureDigest, MaxItems: projection.MaxItems, MaxBytes: projection.MaxBytes, MaxTokens: projection.MaxTokens, PerItemMaxBytes: projection.PerItemMaxBytes, EstimatorRef: projection.EstimatorRef, CheckedUpperBound: projection.OwnerCheckedAt, NotAfter: projection.ExpiresAt, ProjectionID: projection.Ref.ID, ProjectionRevision: projection.Ref.Revision}
...
func (in CurrentRequestV2) Validate() error {
	...
	for _, ref := range []contract.Ref{in.CurrentStateRef, in.ExpectedQueryRef, in.ExpectedViewRef, in.ExpectedWatermarkRef, in.AuthorityRef, in.PolicyRef, in.EstimatorRef} {
		if err := ref.Validate(); err != nil {
			return err
		}
	}
...
func (in AttemptInspectionV2) Validate() error {
	if in.ContractVersion != ContractVersionV2 || in.ObjectKind != AttemptInspectionKindV2 || in.Owner != contract.OwnerMemory || in.OwnerCheckedAt.IsZero() || !in.ExpiresAt.After(in.OwnerCheckedAt) {
		return contract.ErrInvalidArgument
	}
...
func (in ProjectionItemV2) Validate() error {
	if in.ContractVersion != ContractVersionV2 || in.ObjectKind != ProjectionItemKindV2 || in.Rank < 0 || in.TokenEstimate < 0 || strings.TrimSpace(in.CitationDigest) == "" || strings.TrimSpace(in.AssociationDigest) == "" || in.ExpiresAt.IsZero() {
		return contract.ErrInvalidArgument
	}
...
func (in CurrentProjectionV2) Validate() error {
	if in.ContractVersion != ContractVersionV2 || in.ObjectKind != CurrentProjectionKindV2 || in.Owner != contract.OwnerMemory || !in.Current || in.OwnerCheckedAt.IsZero() || !in.ExpiresAt.After(in.OwnerCheckedAt) || (in.CheckPhase != CheckPhaseS1V2 && in.CheckPhase != CheckPhaseS2V2) {
		return contract.ErrInvalidArgument
	}
	if err := in.Ref.Validate(); err != nil {
		return err
	...
	for _, ref := range []contract.Ref{in.AttemptInspectionRef, in.CurrentStateRef, in.StatePlaneBindingRef, in.QueryRef, in.ViewRef, in.WatermarkRef, in.AuthorityRef, in.PolicyRef, in.EstimatorRef} {
		if err := ref.Validate(); err != nil {
			return err
		}
	}
...
func (in ExactContentObservationV2) Validate() error {
	if in.ContractVersion != ContractVersionV2 || in.ObjectKind != ExactContentObservationKindV2 || in.Owner != contract.OwnerMemory || in.Rank < 0 || in.ObservedLength < 0 || in.OwnerObservedAt.IsZero() || !in.ExpiresAt.After(in.OwnerObservedAt) || in.ObservedDigest != in.ContentRef.Digest || in.ObservedMediaType != in.ContentRef.MediaType || (in.CheckPhase != CheckPhaseS1V2 && in.CheckPhase != CheckPhaseS2V2) {
		return contract.ErrInvalidArgument
	}
	if err := in.Coordinate.Validate(); err != nil {
		return err
	}
	for _, ref := range []contract.Ref{in.Ref, in.ProjectionRef, in.StatePlaneBindingRef, in.RecordRef, in.DomainResultRef, in.ApplicationRef} {
		if err := ref.Validate(); err != nil {
			return err
		}
	}
...
func populateProjectionDigestsV2(p *CurrentProjectionV2) error {
	...
	for i, item := range p.Items {
		...
		stable[i] = stableItemV2{ContractVersion: item.ContractVersion, ObjectKind: item.ObjectKind, Rank: item.Rank, Score: item.Score, RecordRef: item.RecordRef, ContentRef: item.ContentRef, SourceRefs: contract.NormalizeRefs(item.SourceRefs), EvidenceRefs: contract.NormalizeRefs(item.EvidenceRefs), ProjectionRefs: contract.NormalizeRefs(item.ProjectionRefs), CitationDigest: item.CitationDigest, DomainResultRef: item.DomainResultRef, AssociationDigest: item.AssociationDigest, ApplicationRef: item.ApplicationRef, TokenEstimate: item.TokenEstimate, EstimatorRef: item.EstimatorRef}
	...
	p.OrderedItemSetDigest, err = contract.Digest(itemSetBodyV2{MemorySetDigestDomainV2, MemorySetDigestContractVersionV2, MemoryOrderedItemSetObjectKindV2, stable})
	if err != nil {
		return err
	}
	p.ContentSetDigest, err = contract.Digest(contentSetBodyV2{MemorySetDigestDomainV2, MemorySetDigestContractVersionV2, MemoryContentSetObjectKindV2, contents})
	if err != nil {
		return err
	}
	p.SourceSetDigest, err = contract.Digest(refSetBodyV2{MemorySetDigestDomainV2, MemorySetDigestContractVersionV2, MemorySourceSetObjectKindV2, sources})
	if err != nil {
		return err
	}
	p.EvidenceSetDigest, err = contract.Digest(refSetBodyV2{MemorySetDigestDomainV2, MemorySetDigestContractVersionV2, MemoryEvidenceSetObjectKindV2, evidence})
	if err != nil {
		return err
	}
	p.ProjectionSetDigest, err = contract.Digest(refSetBodyV2{MemorySetDigestDomainV2, MemorySetDigestContractVersionV2, MemoryProjectionSetObjectKindV2, projections})
	if err != nil {
		return err
	}
	p.CitationSetDigest, err = contract.Digest(stringSetBodyV2{MemorySetDigestDomainV2, MemorySetDigestContractVersionV2, MemoryCitationSetObjectKindV2, citations})
	if err != nil {
		return err
	}
	c := p.Coordinate
	body := stableClosureV2{MemoryStableClosureDigestDomainV2, MemoryStableClosureContractVersionV2, MemoryStableClosureObjectKindV2, p.Owner, c.TenantID, c.IdentityRef, c.IdentityEpoch, c.ExecutionScopeDigest, c.RunID, c.SessionRef, c.SessionEvidenceRef, c.SourceTurnOrdinal, c.SourceTurnRef, c.TurnEvidenceRef, c.LegacyTurnID, c.AttemptRef, c.RequestDigest, c.IdempotencyKey, c.ObservationRef, c.ResultRef, p.CurrentStateRef, p.StatePlaneBindingRef, p.QueryRef, p.ViewRef, p.WatermarkRef, p.AuthorityRef, p.AuthorityEpoch, p.PolicyRef, p.Purpose, p.Scopes, p.SensitivityMax, normalizeCoverageV2(p.Coverage), p.NextCursor, p.ResultDigest, p.EvidenceDigest, stable, p.OrderedItemSetDigest, p.ContentSetDigest, p.SourceSetDigest, p.EvidenceSetDigest, p.ProjectionSetDigest, p.CitationSetDigest, p.MaxItems, p.MaxBytes, p.MaxTokens, p.PerItemMaxBytes, p.EstimatorRef}
...
func sameProjectionStableV2(a, b CurrentProjectionV2) bool {
	return a.StableClosureDigest == b.StableClosureDigest && a.OrderedItemSetDigest == b.OrderedItemSetDigest && a.ContentSetDigest == b.ContentSetDigest && a.SourceSetDigest == b.SourceSetDigest && a.EvidenceSetDigest == b.EvidenceSetDigest && a.ProjectionSetDigest == b.ProjectionSetDigest && a.CitationSetDigest == b.CitationSetDigest
}
