...
func memoryJob(f *fixture, id string) contract.OwnerJobAttemptV1 {
	return contract.OwnerJobAttemptV1{
		Ref: contract.Ref{ID: id, Revision: 1}, Owner: contract.OwnerMemory, Kind: contract.JobMemoryConsolidation,
		TenantID: f.access.TenantID, AuthorityRef: f.access.AuthorityRef, PolicyRef: f.access.PolicyRef, ScopeRef: ref("scope-a"),
		OperationRef: ref("operation-job"), AttemptRef: ref("attempt-job"), SubjectRef: ref("record-a"), InputDigest: "sha256:input",
		State: contract.JobReserved, CreatedAt: f.now, UpdatedAt: f.now, ExpiresAt: f.now.Add(time.Hour),
	}
}

func TestMemoryOwnerJobCASUnknownInspectAndIsolation(t *testing.T) {
	f := newFixture(t)
	reserved, err := f.store.ReserveJob(f.access, memoryJob(f, "job-a"))
	if err != nil {
		t.Fatal(err)
	}
	begun := reserved
...
func TestMemoryOwnerJobConcurrentCAS(t *testing.T) {
	f := newFixture(t)
	reserved, err := f.store.ReserveJob(f.access, memoryJob(f, "job-concurrent"))
	if err != nil {
		t.Fatal(err)
	}
	next := reserved
