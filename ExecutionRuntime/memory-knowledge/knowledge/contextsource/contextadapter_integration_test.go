...
import (
	...
	"github.com/Proview-China/rax/ExecutionRuntime/memory-knowledge/knowledge/contextadapter"
	"github.com/Proview-China/rax/ExecutionRuntime/memory-knowledge/knowledge/contextsource"
	"github.com/Proview-China/rax/ExecutionRuntime/runtime/core"
)

func TestKnowledgeApplicationAdapterPreservesExactSourceChainAndLicense(t *testing.T) {
	fixture := contextsource.NewAdapterTestFixtureV2(t)
	adapter, err := contextadapter.NewAdapterV1(fixture.Reader, fixture.Now)
	if err != nil {
		t.Fatal(err)
	}
	request := knowledgeApplicationRequestV1(t, fixture.Request)
	s1, err := adapter.InspectContextOwnerSourceCurrentV1(context.Background(), request)
	if err != nil || s1.Owner != applicationcontract.ContextOwnerKnowledgeV1 || len(s1.Items) != 1 || s1.Items[0].License != "internal-use" || len(s1.Items[0].StableOwnerChain) < 4 {
	...
	if err != nil || string(body) != "owner local knowledge content" || observation.ProjectionItemDigest != s2.Items[0].ItemDigest {
	...
	tampered.Envelope.Items[0].License = "forbidden"
...
func knowledgeApplicationRequestV1(t *testing.T, owner contextsource.CurrentRequestV2) applicationcontract.ContextOwnerSourceRequestV1 {
	t.Helper()
	...
	request, err := applicationcontract.SealContextOwnerSourceRequestV1(applicationcontract.ContextOwnerSourceRequestV1{
		Owner:                applicationcontract.ContextOwnerKnowledgeV1,
