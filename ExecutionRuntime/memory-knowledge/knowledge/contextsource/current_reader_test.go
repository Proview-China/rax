...
func newReaderFixture(t *testing.T) *readerFixture {
	...
	binding, err := NewStatePlaneBinding("knowledge/state-plane-binding", 1, "knowledge-owner-state-plane", f.now.Add(20*time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	...
	contentRef := f.content.put([]byte("owner local knowledge content"))
	resultRef := testRef("knowledge/retrieval-result")
	domainResult, err := contract.NewDomainResultFact(contract.OwnerKnowledge, "knowledge/domain-result", "knowledge/local-attempt", testRef("knowledge/operation"), resultRef, testRef("knowledge/inspection"), 0, 1, nil, contract.Coverage{Status: contract.CoverageComplete, Expected: 1, Available: 1}, "local_complete", nil, f.now)
	if err != nil {
		t.Fatal(err)
	}
	...
	application, err := contract.NewSettlementApplication(contract.OwnerKnowledge, "knowledge/application", 1, domainResult, association, contract.RuntimeSettlementRef{Ref: testRef("knowledge/runtime-settlement")}, f.now)
	if err != nil {
		t.Fatal(err)
	}
	recordRef := testRef("knowledge/record")
	packageRef := testRef("knowledge/package")
	snapshotRef := testRef("knowledge/snapshot")
	pointerRef := testRef("knowledge/pointer")
	projectionRef := testRef("knowledge/projection")
	sourceRef := testRef("knowledge/source")
	viewRef := testRef("knowledge/view")
	f.attempt, err = f.store.PutAttempt(LocalAttempt{
		Ref: contract.Ref{ID: "knowledge/local-attempt", Revision: 1}, TenantID: "tenant-a", ExecutionScopeDigest: "sha256:execution", RunID: "run-a", TurnID: "turn-1", RequestDigest: "sha256:request", IdempotencyKey: "knowledge-idempotency",
		ObservationRef: testRef("knowledge/observation"), ResultRef: resultRef, QueryRef: testRef("knowledge/query"), ViewRef: viewRef, SnapshotRef: snapshotRef, PointerRef: pointerRef,
		AuthorityRef: testRef("authority"), PolicyRef: testRef("policy"), Purpose: "answer", Scopes: []string{"project-a"}, AllowedLicenses: []string{"internal-use"}, SensitivityMax: "internal",
		...
		Items:        []StoredItem{{Rank: 0, Score: 20, RecordRef: recordRef, PackageRef: packageRef, SnapshotRef: snapshotRef, ContentRef: contentRef, SourceRefs: []contract.Ref{sourceRef}, EvidenceRefs: []contract.Ref{testRef("knowledge/evidence")}, ProjectionRefs: []contract.Ref{projectionRef}, CitationDigest: testRef("citation").Digest, License: "internal-use", TrustState: "source_supported", ConflictGroup: "group-a", RecordExpiresAt: f.now.Add(time.Hour), SourceExpiresAt: f.now.Add(time.Hour), ProjectionExpires: f.now.Add(time.Hour)}},
	...
	f.state, err = f.store.PublishCurrent(CurrentState{
		Ref: contract.Ref{ID: "knowledge/current", Revision: 1}, TenantID: "tenant-a", AuthorityRef: testRef("authority"), PolicyRef: testRef("policy"), Purpose: "answer", Scopes: []string{"project-a"}, AllowedLicenses: []string{"internal-use"}, SensitivityMax: "internal", ViewRef: viewRef, SnapshotRef: snapshotRef, PointerRef: pointerRef,
		Items: []CurrentItem{{RecordRef: recordRef, PackageRef: packageRef, SnapshotRef: snapshotRef, ContentRef: contentRef, SourceRefs: []contract.Ref{sourceRef}, ProjectionRefs: []contract.Ref{projectionRef}, License: "internal-use", TrustState: "source_supported", ConflictGroup: "group-a", Active: true, PoisoningCleared: true, RecordExpiresAt: f.now.Add(time.Hour), SourceExpiresAt: f.now.Add(time.Hour), ProjectionExpires: f.now.Add(time.Hour)}}, ExpiresAt: f.now.Add(time.Hour),
	}, contract.ExpectAbsent())
	if err != nil {
		t.Fatal(err)
	}
	f.request = CurrentRequest{ContractVersion: ContractVersion, Coordinate: coordinateFrom(f.attempt), RunID: "run-a", TurnID: "turn-1", CurrentStateRef: f.state.Ref, AuthorityRef: testRef("authority"), PolicyRef: testRef("policy"), Purpose: "answer", Scopes: []string{"project-a"}, AllowedLicenses: []string{"internal-use"}, SensitivityMax: "internal", CheckedAt: f.now, NotAfter: f.now.Add(30 * time.Minute), ProjectionID: "knowledge/contribution", ProjectionRevision: 1}
	return f
}
...
func TestKnowledgeCurrentReaderExactCanonicalAndCopyIsolation(t *testing.T) {
	...
	if err != nil || !projection.Current || projection.Ref.Validate() != nil || len(projection.Items) != 1 || projection.Items[0].License != "internal-use" {
		t.Fatalf("projection=%+v err=%v", projection, err)
	}
	observation, body, err := f.store.ReadContentExact(exactRequest(f, projection, f.now.Add(20*time.Minute)))
	if err != nil || observation.Ref.Validate() != nil || observation.License != "internal-use" || string(body) != "owner local knowledge content" {
		t.Fatalf("observation=%+v body=%q err=%v", observation, body, err)
	}
	projection.Items[0].SourceRefs[0] = testRef("mutated")
	projection.AllowedLicenses[0] = "mutated"
	body[0] = 'X'
	again, err := f.store.InspectForTurn(f.request)
	if err != nil || again.AllowedLicenses[0] != "internal-use" || again.Items[0].SourceRefs[0].ID != "knowledge/source" {
	...
	if err != nil || string(againBody) != "owner local knowledge content" {
		t.Fatalf("caller mutated local content: %q %v", againBody, err)
	}
}

func TestKnowledgeCurrentReaderFailClosedCases(t *testing.T) {
	...
	t.Run("historical pointer is not current", func(t *testing.T) {
		f := newReaderFixture(t)
		next := cloneCurrentState(f.state)
		next.Ref = contract.Ref{ID: f.state.Ref.ID, Revision: 2}
		next.PointerRef = testRefRevision("knowledge/pointer", 2)
		next.SnapshotRef = testRefRevision("knowledge/snapshot", 2)
		next.Items[0].SnapshotRef = next.SnapshotRef
		if _, err := f.store.PublishCurrent(next, contract.ExpectRevision(1)); err != nil {
			t.Fatal(err)
		}
		if _, err := f.store.InspectForTurn(f.request); !errors.Is(err, contract.ErrNotCurrent) {
			t.Fatalf("historical pointer accepted: %v", err)
		}
	})
	t.Run("withdrawn stale and license drift", func(t *testing.T) {
		f := newReaderFixture(t)
		next := cloneCurrentState(f.state)
		next.Ref = contract.Ref{ID: f.state.Ref.ID, Revision: 2}
		next.Items[0].Withdrawn = true
		next.Items[0].ProjectionRefs = []contract.Ref{testRefRevision("knowledge/projection", 2)}
		next.Items[0].License = "revoked"
		...
		if _, err := f.store.InspectForTurn(req); !errors.Is(err, contract.ErrNotCurrent) {
			t.Fatalf("withdrawn/stale/license drift accepted: %v", err)
		}
	})
	...
	t.Run("current state canonical tamper", func(t *testing.T) {
		...
		versions[0].Items[0].License = "tampered"
	...
	t.Run("settlement association drift", func(t *testing.T) {
		...
		bad.Association.DomainResultRef = testRef("knowledge/wrong-domain-result")
...
func TestKnowledgeCurrentReader64ConcurrentConsistentSnapshot(t *testing.T) {
	f := newReaderFixture(t)
	const workers = 64
	...
	for i := 0; i < workers; i++ {
		...
		go func(i int) {
			...
			if !contract.SameRef(projection.Items[0].RecordRef, f.attempt.Items[0].RecordRef) || !contract.SameRef(projection.PointerRef, f.attempt.PointerRef) {
				errs <- fmt.Errorf("mixed snapshot: %+v", projection)
				return
			}
			...
			if err != nil || string(body) != "owner local knowledge content" {
	...
	next.PointerRef = testRefRevision("knowledge/pointer", 2)
	next.SnapshotRef = testRefRevision("knowledge/snapshot", 2)
	next.Items[0].RecordRef = testRefRevision("knowledge/record", 2)
	next.Items[0].SnapshotRef = next.SnapshotRef
	next.Items[0].ContentRef = f.content.put([]byte("new current knowledge"))
	if _, err := f.store.PublishCurrent(next, contract.ExpectRevision(1)); err != nil {
		t.Fatal(err)
	}
	wg.Wait()
