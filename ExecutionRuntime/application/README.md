...
- TurnContinuationCoordinatorV1闭合已结算ToolResult之后的最小Tool-only续轮顺序：Harness持久Begin(pending)→Context协调器发布exact refresh→Harness以Expected ActiveContext做Commit(CAS)→仅对exact context_current开放下一Model Turn。V1会在Begin前重算sealed Context Attempt并拒绝same Attempt不同payload；Memory/Knowledge owner-backed refresh因无法在final replay早退前独立证明完整payload，固定CapabilityUnavailable。该协调器的注入面只有Harness continuation Port、Context coordination Port和Clock，不拥有或调用Model、Tool、Provider；Harness mutation回包未知只Inspect原Attempt，不重派。多个Application coordinator和多个Harness SQLite handle可共享一个Context coordinator并发收敛到同一successor；Context coordinator当前single-call gate仅为进程内事实，多个独立Context composition root/跨进程同Attempt并发仍为NO-GO，不得宣称集群级Context refresh已闭合。
