...
状态：Owner-local已完成的软件切面均通过验证。CTX-D09 A层Refresh、CTX-D10 Reader、Offline SDK、`ContextOfflineIngressV1`、`ContextEngineeringSDKV1`、`ContextRuntimeAPIV1` typed facade、`ContextLocalCacheFacadeV1`、Compaction/Generation、Outcome事实链、Recipe/PromptAsset pre-release及`PromptUpstreamProvenanceV1`离线核已经落地。Runtime API仅封装immutable Frame consumption、settled Tool Result append及Context-owned descriptor的exact bounded materialization；`MaterializeDescriptor`在读取前执行数量、单项和总量限制，只返回经过长度/digest复核的深拷贝中立区域，不跟随Artifact正文，也不声明Provider、Model、Current或Host composition权威。Local Cache Facade统一Fragment/Frame/Projection typed ports，提供有界attempt恢复、确定性同步逐出和process-local telemetry；全部缓存均可丢失、非权威，不产生Current Fact，也不自动read-through。Engineering五入口API/CLI和Prompt Provenance均通过target100/race20及full ordinary/race/vet。production发布、跨Owner root、Capability、Harness Continuation与Turn推进仍未启用。
