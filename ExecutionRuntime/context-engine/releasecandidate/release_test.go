...
import (
	...
	"github.com/Proview-China/rax/ExecutionRuntime/context-engine/releasecandidate"
	"github.com/Proview-China/rax/ExecutionRuntime/harness/assemblycontract"
	"github.com/Proview-China/rax/ExecutionRuntime/runtime/core"
	runtimeports "github.com/Proview-China/rax/ExecutionRuntime/runtime/ports"
)

var testNow = time.Date(2026, 7, 18, 1, 10, 0, 0, time.UTC)
...
func validRequest() releasecandidate.RequestV1 {
	return releasecandidate.RequestV1{
		ReleaseID: "release/context/assembly-candidate", Revision: 1,
		ArtifactDigest: digest("context-artifact-v1"), SourceRef: ref("source/context/v1"),
		PublisherRef: ref("publisher/context-owner"), TrustRef: ref("trust/context-owner"),
		EvidenceRefs: []assemblycontract.ObjectRefV1{ref("evidence/context-offline"), ref("evidence/context-refresh"), ref("evidence/context-current")},
		TTL:          time.Hour,
	}
}
...
func TestBuildV1ClosesReferenceOnlyRelease(t *testing.T) {
	candidate := build(t)
	...
	if !candidate.Conformance.ReferenceOnly || candidate.Conformance.ProductionSLA || candidate.Conformance.DurableState || candidate.Conformance.DurableCache || candidate.Conformance.ProviderCalls != 0 || candidate.Conformance.HarnessContinuation {
		t.Fatalf("owner-local conformance drifted: %+v", candidate.Conformance)
	}
...
func TestProductionPromotionFailsClosed(t *testing.T) {
	...
	if _, err = assemblercontract.SealComponentReleaseV1(release); !core.HasReason(err, core.ReasonBindingNotCertified) {
		t.Fatalf("reference Context release promoted: %v", err)
	}
}
...
func (s *releaseStore) EnsureExactComponentReleaseV1(_ context.Context, value assemblercontract.ComponentReleaseV1) (assemblercontract.ComponentReleaseV1, error) {
	...
	if s.driftReturn {
		...
		drifted.ReleaseID = "release/context/drifted"
...
func TestRequestTTLArtifactConformanceAndProofDriftFailClosed(t *testing.T) {
	tests := []struct {
		name   string
	...
	candidate := build(t)
	candidate.Readiness.MissingProductionProofs[0] = "context.fake-proof"
	...
	candidate = build(t)
	candidate.Conformance.DurableCache = true
	if err := candidate.ValidateCurrentV1(testNow.Add(time.Minute)); !core.HasReason(err, core.ReasonBindingNotCertified) {
		t.Fatalf("conformance drift accepted: %v", err)
	}
	candidate = build(t)
