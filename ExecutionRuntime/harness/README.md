...
- `bridgecontract`：仅承载Application V3与Harness之间的Model Turn Reservation/Binding不可变关联，不进入Harness核心Session语义；
...
- 当前不暴露Checkpoint能力，也不承诺跨Run或跨Instance Session恢复。
...
当前共有120个顶层Test/Fuzz入口，其中4个Fuzz入口；分为公共合同/Port单元测试、Kernel与Adapter白盒测试，以及Runtime Foundation与真实Application Coordinator跨模块黑盒测试。生产包跨包语句覆盖率为79.3%。测试包含跨租户同ID、同Instance顺序Run、自定义组件隔离、原子Reservation、丢回包恢复、模型结果未知、独立Inspect provenance、内部因果换包、关闭后恢复观察与并发竞态。
...
当前实现没有真实Harness Provider、真实模型、真实Tool/MCP、生产Scheduler、数据库、RPC后端或可信Plan Assembler；Binding注册/Probe/Conformance也不产生生产认证或dispatch资格。Application V3到Harness的持久Domain接线与公共Run协调边界已经闭合，但生产Runtime Gateway、Provider和持久后端仍必须由各自Owner实现，不能用测试Port double冒充。公共基座解锁6+1开发不等于生产部署完成。
