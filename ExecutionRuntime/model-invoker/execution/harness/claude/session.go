...
func (session *session) Command(ctx context.Context, command union.ExecutionCommand) error {
	...
	case union.CommandCancelExecution:
		...
		// Publish the PendingCall under the same lock used by Receive. The SDK
		// can answer and finish before BeginCall returns; without this barrier a
		// result could be decoded while cancelPending was still nil, losing the
		// acknowledgement/quiescence evidence required by Runtime.
...
func (session *session) synthesizeMissingResult() {
	...
	session.terminal = true
	payload, _ := json.Marshal(map[string]string{"reason": "eof_without_result"})
	session.queue = append(session.queue,
		...
		terminalDraft(session.nextSequence(), session.init.SessionID, execution.RouteTerminalCandidate{
			Status: union.ExecutionStatusIndeterminate, StopReason: "eof_without_result",
...
func (session *session) decodeSystem(raw json.RawMessage) ([]union.UnifiedExecutionEvent, error) {
	...
	if subtype == "init" {
		return []union.UnifiedExecutionEvent{diagnosticDraft(session.nextSequence(), sessionID, "system/init", "protocol_violation", "duplicate_init", "duplicate init after preflight", unknownPayload(raw, "system", subtype))}, nil
	}
	itemID := objectString(raw, "task_id")
	if itemID == "" {
		itemID = objectString(raw, "uuid")
	}
	if itemID == "" {
		itemID = "claude-system-" + subtype
	}
	switch subtype {
	case "hook_started", "task_started", "task_progress":
		return []union.UnifiedExecutionEvent{itemDraft(session.nextSequence(), sessionID, "system/"+subtype, itemID, subtype, union.ItemStatusInProgress, union.SideEffectPossible, raw)}, nil
	case "hook_response", "task_notification", "task_updated":
		status := union.ItemStatusCompleted
		nativeStatus := objectString(raw, "status")
		if nativeStatus == "failed" {
			status = union.ItemStatusFailed
		} else if nativeStatus == "stopped" || nativeStatus == "killed" {
			status = union.ItemStatusCancelled
		}
		return []union.UnifiedExecutionEvent{itemDraft(session.nextSequence(), sessionID, "system/"+subtype, itemID, subtype, status, union.SideEffectPossible, raw)}, nil
	default:
...
func (session *session) decodeStreamEvent(raw json.RawMessage) ([]union.UnifiedExecutionEvent, error) {
	...
	case "content_block_start":
		...
		return session.decodeBlock(sessionID, "stream_event/content_block_start", objectRaw(eventRaw, "content_block"), index, true)
...
func (session *session) decodeDelta(sessionID string, eventRaw json.RawMessage) ([]union.UnifiedExecutionEvent, error) {
	...
	case "input_json_delta":
		...
		return []union.UnifiedExecutionEvent{modelDraft(session.nextSequence(), sessionID, "stream_event/input_json_delta", "tool_input_delta", nil, "provider_exposed_output", union.ActionID(tool.id), payload)}, nil
	default:
...
func (session *session) selectPlan(toolName string) (union.MechanismPlan, error) {
	...
	switch needle {
	case "bash":
		aliases = append(aliases, "shell", "execute", "code")
	case "edit":
		aliases = append(aliases, "modify", "rewrite", "patch")
	case "write":
		aliases = append(aliases, "create", "rewrite")
	case "read":
		aliases = append(aliases, "inspect")
	}
...
// structuredOutputEvents binds a text/structured ResultMessage to the
// prepared ProduceStructured mechanism even when the Agent used no tool. A
// successful native result is output evidence, but it is not itself an Effect;
// Praxis still owns schema verification and reconciliation.
func (session *session) structuredOutputEvents(status union.ExecutionStatus, hasOutput bool) []union.UnifiedExecutionEvent {
	...
	attempt := union.MechanismAttempt{
		ID: union.MechanismAttemptID("claude-output-attempt-" + string(session.plan.ExecutionID) + "-" + string(selected.ID)), MechanismPlanID: selected.ID,
		...
		NativeToolIdentity: &union.NativeIdentity{Namespace: "anthropic.claude-agent-sdk", Kind: "output", Value: "ResultMessage"},
...
func (session *session) ensureAttempt(toolUseID, toolName string, input json.RawMessage, itemStatus union.ItemStatus) (toolAttempt, []union.UnifiedExecutionEvent, error) {
	...
	sideEffects := toolSideEffects(toolName)
	attempt := union.MechanismAttempt{
		ID: union.MechanismAttemptID("claude-attempt-" + toolUseID), MechanismPlanID: plan.ID,
		Authoritative: true, ActualKind: toolName, ActualOrigin: union.CapabilityOriginHarnessHosted,
		ActualOwner:        union.ExecutionOwnerHarness,
		NativeToolIdentity: &union.NativeIdentity{Namespace: "anthropic.claude-agent-sdk", Kind: "tool", Value: toolName},
		StartedAt:          session.clock().UTC(), Status: union.AttemptStatusRunning,
		SanitizedInput: append(json.RawMessage(nil), input...), SideEffectState: sideEffects,
	}
...
func (session *session) decodeControlRequest(raw json.RawMessage) ([]union.UnifiedExecutionEvent, error) {
	...
	events := make([]union.UnifiedExecutionEvent, 0, 2)
	events = append(events, attemptEvents...)
	expiresAt := session.clock().UTC().Add(session.approvalTTL)
	events = append(events, approvalDraft(session.nextSequence(), session.init.SessionID, requestID, toolUseID, toolName, digest, state.attempt.ID, expiresAt, request))
	return events, nil
}
...
func (session *session) decodeResult(raw json.RawMessage) ([]union.UnifiedExecutionEvent, error) {
	...
	if err := json.Unmarshal(raw, &result); err != nil || result.Subtype == "" || result.IsError == nil {
		return nil, fmt.Errorf("%w: malformed ResultMessage", ErrProtocol)
	}
	...
	if stopReason == "interrupted" || stopReason == "interrupt" || result.Subtype == "interrupted" {
		status = union.ExecutionStatusCancelled
	} else if !*result.IsError && result.Subtype == "success" {
		status = union.ExecutionStatusSucceeded
	} else if *result.IsError && (strings.HasPrefix(result.Subtype, "error_") || result.Subtype == "success") {
		status = union.ExecutionStatusFailed
	}
	outputEvents := session.structuredOutputEvents(status, result.Result != "" || len(result.StructuredOutput) != 0)
	session.mu.Lock()
	...
	resultPayload, _ := json.Marshal(map[string]any{
		"subtype": result.Subtype, "is_error": *result.IsError, "stop_reason": stopReason,
		"result": result.Result, "structured_output": result.StructuredOutput,
		"usage": result.Usage, "permission_denials": result.PermissionDenials, "errors": result.Errors,
	})
	events := append(outputEvents,
		modelDraft(session.nextSequence(), sessionID, "result", "route_result", nil, "provider_exposed_output", "", resultPayload),
		terminalDraft(session.nextSequence(), sessionID, execution.RouteTerminalCandidate{
			Status: status, StopReason: stopReason, PendingBackgroundWork: pending, SideEffectState: sideEffects,
		}),
	)
	return events, nil
}
