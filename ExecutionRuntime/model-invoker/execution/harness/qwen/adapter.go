...
func (adapter *Adapter) Preflight(ctx context.Context, invocation execution.Invocation) (execution.PreflightReport, error) {
	...
	if invocation.Plan.ExecutionKind != union.ExecutionKindAgent || invocation.Plan.Route != adapter.config.Route {
		return rejected("qwen_route_mismatch"), nil
	}
	adapter.mu.Lock()
	...
	if err != nil {
		return execution.PreflightReport{}, fmt.Errorf("%w: SDKSystemMessage: %v", ErrProtocol, err)
	}
	...
	if err := validateInit(adapter.config.ExpectedInit, init); err != nil {
		return rejected("qwen_manifest_drift"), nil
	}
...
func (adapter *Adapter) Open(ctx context.Context, invocation execution.Invocation) (execution.Session, error) {
	...
	if err := prepared.client.Send(buildUserMessage(invocation.Request, prepared.init.SessionID)); err != nil {
		_ = prepared.client.Close()
		return nil, err
	}
	return newSession(
		prepared.client, prepared.init, prepared.manifest, invocation.Plan,
		adapter.config.Clock, adapter.config.ApprovalTTL,
	), nil
}
