...
func eventHeader(family union.EventFamily, sequence uint64, sessionID, nativeType string) union.EventHeader {
	...
	header.NativeIdentity = &union.NativeIdentity{Namespace: "alibaba.qwen-code-sdk", Kind: "message", Value: nativeType}
	return header
}
...
func manifestDraft(sequence uint64, init InitMessage, manifest union.ContextManifestSummary) union.UnifiedExecutionEvent {
	...
	return union.UnifiedExecutionEvent{
		Header:     eventHeader(union.EventFamilyDiagnostic, sequence, init.SessionID, "SDKSystemMessage/init"),
		Diagnostic: &union.DiagnosticEvent{Kind: "actual_manifest", Code: "sdk_system_init", Manifest: &clone, Payload: append(json.RawMessage(nil), init.Raw...)},
	}
}
...
func terminalDraft(sequence uint64, sessionID string, candidate execution.RouteTerminalCandidate) union.UnifiedExecutionEvent {
	...
	return diagnosticDraft(sequence, sessionID, "SDKResult", execution.EventKindRouteTerminalCandidate, candidate.StopReason, "", payload)
}

func unknownPayload(raw json.RawMessage, nativeType, subtype string) json.RawMessage {
	digest, _ := union.StableDigest(raw)
	payload, _ := json.Marshal(map[string]any{"type": nativeType, "subtype": subtype, "digest": digest, "bytes": len(raw)})
	return payload
}
