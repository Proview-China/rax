...
func startRPCWire(ctx context.Context, config harnessprocess.Config) (*rpcWire, error) {
	if config.Protocol != harnessprocess.ProtocolJSONRPCNDJSON {
		return nil, fmt.Errorf("%w: process protocol must be jsonrpc_ndjson", ErrInvalidConfig)
	}
	processSession, err := harnessprocess.Start(ctx, config)
	if err != nil {
		return nil, err
	}
	wire := &rpcWire{process: processSession, pending: make(map[string]chan wireResponse), inbound: make(chan wireEnvelope, 128), done: make(chan struct{})}
	go wire.readLoop()
	return wire, nil
}
...
func (w *rpcWire) readLoop() {
	for {
		...
		if message.Kind == harnessprocess.JSONRPCResponse {
			...
			response <- wireResponse{result: cloneRaw(message.Result), fault: cloneRaw(message.Error)}
			continue
		}
		select {
		case w.inbound <- wireEnvelope{message: message, raw: cloneRaw(frame.Raw)}:
		case <-w.done:
			return
		}
	}
...
func (w *rpcWire) respond(id, result, fault json.RawMessage) error {
	...
	payload := map[string]json.RawMessage{"jsonrpc": json.RawMessage(`"2.0"`), "id": cloneRaw(id)}
	if len(result) != 0 {
