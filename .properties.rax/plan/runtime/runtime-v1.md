...
- 状态：已通过独立文件复审；用户授权的Runtime公共合同与组件中立最小基座已完成，其余决策继续按阶段确认；
- 实现授权：公共对象、纯Runtime Kernel、版本化Port、组件注册与装配门禁、最小可运行协调器、fake端口、完整闭环测试与说明资产；不含相邻组件内部实现和生产后端；
...
19. Timeline/Checkpoint/Restore、单Run、组件Registry和按Port接入后的统一闭环验收框架。
...
状态：**已完成最小基座（2026-07-14）**；“逐组件接入”从后续每个组件任务重复执行。

- 实现不依赖真实组件的Foundation Coordinator；
- 实现Runtime所有的持久化Run Fact/CAS合同与崩溃后Inspect恢复；
- 实现Harness Completion Claim的source sequence幂等证据摄取，并保持Claim与Runtime权威Outcome分离；
- 使用fake Execution、Environment、Evidence和Checkpoint Port跑通Admission→Activation→Run→Checkpoint→Stop→Cleanup；
- 提供Component Registry和ResolvedAgentPlan装配校验；
- 把每个后续组件的接入验收固定为同一套Descriptor、能力、Effect、故障、Checkpoint和闭环测试；
- Rewind只允许从consistent Checkpoint创建新Instance和更高epoch。

完成条件：两种内部行为不同的fake Execution Adapter通过同一闭环；Required缺失、能力过期、Ready冲突、Partial Checkpoint和不完整Cleanup全部fail closed。

## 7. 细粒度检查清单

### 身份与状态

- [x] IdentityExecutionLease单活跃保证；
- [x] proposed→reserved→ActivationCommit→active无循环顺序和逐步恢复；
- [x] Lineage固定Plan Digest；
- [x] 重建新Instance ID/epoch；
- [x] V1单活跃Run；
- [x] Run Fact持久化、revision CAS、写入回包丢失恢复与冲突终态隔离；
- [x] 三维状态和分维度终止；
- [ ] 合法组合谓词、迁移前置条件、失败落点与迟到隔离；
- [x] Execution不等于Task/Artifact成功。
...
- [x] Intent-before-dispatch；
- [x] UnknownOutcome不盲重试；
- [ ] Budget原子Reservation；
- [ ] Hard Budget只有存在可执行有限cap时成立；
- [ ] RemoteContinuation独立清理；
- [x] Harness自报不成为权威事实；
- [x] Evidence不可用时阻断新Effect；
...
1. **已确认基座**：Runtime代码位置为`ExecutionRuntime/runtime`，技术语言为Go；后续语言扩展另行审核；
