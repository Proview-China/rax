...
| `xai.api` | xAI / 按量 API | Responses已实现；原生 gRPC明确延期；不声明 legacy Chat | Go Responses协议驱动已完成 `grok-4.5`离线实现；Python `xai-sdk`/公开 proto只作为未来 gRPC专项证据，不改变当前 Adapter状态 | `implemented_offline` | `fresh` |
...
| `anthropic.consumer-plans.messages-api` | Claude Pro/Max消费者订阅 | Claude.ai产品登录不提供 Messages API、Bedrock或 Vertex额度 | 明确阻断把消费者套餐当作 API Credential或消费计划 | `fresh + research_only + non_callable` |
| `anthropic.consumer-plans.agent-sdk` | Claude Pro/Max消费者订阅下的 Agent SDK合同 | 独立 Agent SDK登录/宿主合同，不是 Messages API Key或 Base URL替换 | 官方允许第三方应用通过 Agent SDK使用套餐；Praxis尚无经确认的 SDK/sidecar/auth设计，只保留P1研究轴 | `fresh + research_only + design_required` |
...
`catalog.DefaultDocument`当前包含62条记录：39条默认callable Binding、16条已实现但host-blocked的订阅Route和7条研究/控制记录。下表只渲染当前默认可调用的39条精确Binding；host-blocked与研究/控制记录都不会进入生成区块。
...
| `deepseek.direct.payg.chat_completions` | `deepseek` | `deepseek` | `deepseek.api.payg` | `deepseek.direct.global` | `chat_completions` | `https://api.deepseek.com` | `deepseek.default.openai` | `fresh` | `implemented_offline` |
| `deepseek.direct.payg.messages` | `deepseek` | `deepseek` | `deepseek.api.payg` | `deepseek.direct.global` | `messages` | `https://api.deepseek.com/anthropic` | `deepseek.default.anthropic` | `fresh` | `implemented_offline` |
...
其余直连、订阅、云托管和第三方路线仍保持本矩阵各节记录的真实状态。默认Catalog中的16条受限订阅Route为`callable=false`但保留已实现Adapter ID和`blocked_by_host_trust`标记；其余研究/控制记录仍无Adapter。两者都不因可查询而成为默认可调用Provider。
