...
| 集成与真实烟测入口 | `ExecutionRuntime/model-invoker/tests/integration/` | Codex/Claude/Gemini/Kimi/Qwen生产Harness Adapter + fake child process离线Runtime集成；另含Provider直连、十家P0 Gateway与Kimi Code/MiniMax Token两类P1的显式双开关真实入口，MiMo/Alibaba禁止测试器 |
| 统一离线入口 | `ExecutionRuntime/model-invoker/scripts/verify-offline.sh` | 屏蔽真实凭据后执行格式、模块、静态、普通、shuffle、race、完整integration-tag离线套件（真实smoke默认Skip）和Catalog资产门禁 |
...
- `UnifiedExecutionRequest` 与 `PreparedExecutionPlan` 通过 request/plan digest、IntentGraph、Profile/Route 期望值封装，封装后篡改会在 Adapter 前拒绝；
- Direct OpenAI 与 Codex App Server、Claude Agent SDK、Gemini ACP、current Kimi ACP、Qwen SDK 六路已通过离线 Profile 编译和统一 Runtime 收敛测试；各路保留自己的 Mechanism/Origin/Owner，最终 Effect、Verification 与 Satisfaction 一致；
- Harness 只能产生 Mechanism、Item、Control 和 route terminal candidate；Effect、Verification 与唯一统一终态继续由 Praxis Runtime 独占；
- 执行并集第二轮Review已通过普通、五轮全仓shuffle、全仓Race/Vet、完整integration-tag离线套件、五项3秒Fuzz与四项基准；默认全仓覆盖率`76.6%`，合并integration profile为`76.7%`；真实 API/OAuth/订阅状态仍为 `not_run`；
...
- 统一离线入口实际通过，CI已接入同一入口；覆盖 `go mod verify`、gofmt、tidy diff、`git diff --check`、vet、普通、shuffle、race、完整integration-tag离线套件（真实smoke默认Skip）和Catalog资产校验；
...
执行并集 Runtime、六路 Adapter、黑白盒、跨路由集成与全量离线验收记录在[执行语义并集 Runtime 离线实现与验收完成快照](../../memory/model-invoker/20260713-041100-执行语义并集Runtime离线实现与验收完成.md)中。
...
真实认证、付费调用、生产评审、全局Runtime Kernel、Context Engine、全局`profile-system`集成和缓存策略仍需独立授权；当前已实现的Semantic Route Profile离线结果不代表真实模型可用性或生产支持。
